...
```yaml
course_code: "PSM"          # Unique code (prefer existing naming pattern; may derive from `code` if present)
...
- Contains ordered sessions with: title, goal (outcome-focused), learning resources, activities, assignment (outcome-based), reflection components / prompts.
...
3. Validate assignments state outcomes ("Participants will be able to…")—explicitly outcome-focused, not output tasks.
4. Commit with message: `chore(course): update syllabus <course_code>`.

## 6. Issue Template Workflow

Use `.github/ISSUE_TEMPLATE/update-syllabus.yml` (exact path):
