...
- `docs/editorial-terminology-glossary.md` — Authoritative source for all terminology standards
...
- NO system-managed fields created or modified (tags, categories, concepts, Watermarks, aliasesArchive, ItemId, ResourceId, ResourceType, ResourceImportId)
- Terminology compliance confirmed against glossary
- Technical accuracy validated
- Voice and tone alignment checked
...
When reviewing content where the **title** is changed but no explicit **slug** field exists in the front matter, the old title-derived URL must be added to the `aliases` array to preserve existing links and SEO. Hugo auto-generates slugs from titles, so changing a title without a slug changes the URL.

**Example:**

```yaml
# If changing title AND no slug field exists, MUST add to aliases:
aliases:
  - /blog/the-urgent-call-for-agile-organisational-transformation
```

When slug field IS present, title changes do NOT require alias updates (the slug field controls the URL, not the title).
...
**Build validation:** Always validate changes with `hugo build --source site --config hugo.yaml,hugo.local.yaml`

Common Hugo issues to watch for:
