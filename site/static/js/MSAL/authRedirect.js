...
myMSALObj
  ...
  .then((response) => {
    if (response) {
      handleResponse(response); // Handle the response if it exists
    } else {
      selectAccount(); // Select an account if no response
    }
  })
  .catch((error) => {
    console.error("Error handling redirect:", error); // Log any errors
  });
